/*
 * STORYLINE QUIZ EXTRACTOR v8.0
 * Works on Articulate Storyline courses
 *
 * Features:
 * - AUTO-RUNS on paste - no need to call any functions
 * - Multi-window/popup support (finds content in parent, opener, iframes)
 * - Filter out navigation buttons (Continue, Next, Back, etc.)
 * - Deduplicate answers (no more duplicate entries)
 * - Expanded question detection and answer types
 * - Expanded correct answer indicators
 *
 * Usage:
 * 1. Open course in browser
 * 2. Open DevTools Console (F12)
 * 3. Paste this script - it runs automatically!
 */

(async function StorylineExtractor() {
    console.log('╔═══════════════════════════════════════════════════════════╗');
    console.log('║         STORYLINE QUIZ EXTRACTOR v8.0                     ║');
    ...
    const CONFIG = {
        ...
        // Navigation/UI button text to EXCLUDE from answers
        excludeButtonText: [
            'continue',
            'next',
            'back',
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // WINDOW MANAGEMENT - Find content across windows/frames/popups
    // ═══════════════════════════════════════════════════════════════════════════

    function getAllAccessibleWindows() {
        const windows = new Map();
        const visited = new WeakSet();

        function addWindow(win, source) {
        ...
        for (let i = 0; i < 10 && opener; i++) {
            ...
            try { opener = opener.opener; } catch (e) { break; }
        }

        // Collect iframes from all windows
                    ...
                    try {
                        ...
                        if (frameWin && !visited.has(frameWin)) {
                            ...
                            try {
                                ...
                                windows.set(url, { window: frameWin, source: `frame_${idx}` });
                            } catch (e) { /* cross-origin */ }
                        }
                    } catch (e) { /* frame not accessible */ }
                });
            } catch (e) { /* window not accessible */ }
        }
    ...
    const windowContext = {
        ...
        isMain: window === window.top && !window.opener
    };
    console.log('[Context]', windowContext);

    const allWindows = getAllAccessibleWindows();
    console.log(`[WindowManager] Found ${allWindows.size} accessible window(s)`);

    // ═══════════════════════════════════════════════════════════════════════════
    // STEP 1: Find base URL automatically (searches ALL windows)
    // ═══════════════════════════════════════════════════════════════════════════

    let baseUrl = null;

    // Search all accessible windows for Storyline scripts
    for (const [url, { window: win, source }] of allWindows) {
        try {
            // Check performance entries
            if (win.performance?.getEntriesByType) {
                const scripts = win.performance.getEntriesByType('resource').map(r => r.name);
                const courseScript = scripts.find(s => s.includes('/html5/') || s.includes('/story_content/'));
                if (courseScript) {
                    baseUrl = courseScript.match(/(.*?)\/html5\//)?.[1] ||
                              courseScript.match(/(.*?)\/story_content\//)?.[1];
                    if (baseUrl) {
                        console.log(`[Found] Base URL from ${source}:`, baseUrl);
                        break;
                    }
                }
            }
            ...
            // Check script tags
            win.document.querySelectorAll('script[src]').forEach(s => {
                if (!baseUrl && s.src.includes('/html5/')) {
                    baseUrl = s.src.match(/(.*?)\/html5\//)?.[1];
                    if (baseUrl) console.log(`[Found] Base URL from script in ${source}:`, baseUrl);
                }
            });
        } catch (e) { /* window not accessible */ }
    }
    ...
    if (!baseUrl) {
        ...
        if (currentUrl.includes('/html5/')) {
            baseUrl = currentUrl.match(/(.*?)\/html5\//)?.[1];
        } else if (currentUrl.includes('/story')) {
            baseUrl = currentUrl.replace(/\/story.*$/, '');
        }
    }
    ...
    console.log('║   window.windowContext   - window/frame context           ║');
    console.log('╠═══════════════════════════════════════════════════════════╣');
    console.log('║ Commands:                                                 ║');
    console.log('║   exportQA("json")       - download as JSON               ║');
    console.log('║   exportQA("txt")        - download readable format       ║');
    console.log('╚═══════════════════════════════════════════════════════════╝');
    ...
    function extractQA(obj, slideId, config, stats) {
        ...
        function search(obj, path = '') {
            ...
            if (isAnswerType(accType) && text && text.length >= config.minAnswerLength) {
                // FILTER: Skip navigation/UI buttons
                let lowerText = text.toLowerCase().trim();
                if (accType === 'button' && config.excludeButtonText.some(exc =>
                    lowerText === exc || lowerText.includes(exc)
                )) {
                    if (config.verbose) {
                        console.log(`  Skipped nav button: "${text}"`);
                    }
                    stats.skipped.push({ text, reason: 'navigation button', accType });
                    return; // Skip this element
                }

                // FILTER: Skip duplicates (same text already exists)
                if (result.answers.some(a => a.text === text)) {
                    if (config.verbose) {
                        console.log(`  Skipped duplicate: "${text}"`);
                    }
                    stats.skipped.push({ text, reason: 'duplicate', accType });
            ...
            if (accType === 'text' || !accType) {
                if (isQuestionText(text)) {
                    // Don't use text that's already an answer as the question
                    if (result.answers.some(a => a.text === text)) {
                        if (config.verbose) {
                            console.log(`  Skipped as question (is answer): "${text.substring(0, 50)}..."`);
                        }
                    }
                    // Keep the longest/most complete question text
                    else if (!result.question || text.length > result.question.length) {
