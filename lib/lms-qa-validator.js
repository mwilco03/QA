    ...
    const SCORMAPI = {
        ...
        async markAllSlidesViewed(options = {}) {
            ...
            try {
                ...
                switch (detectedTool) {
                    case 'storyline':
                        ({ data: newSuspendData, ...slideInfo } = this._markStorylineSlides(suspendData));
                        // Also set Storyline variables
                        this._setStorylineSlideVariables();
                        break;

                    case 'rise':
                        ({ data: newSuspendData, ...slideInfo } = this._markRiseBlocks(suspendData));
                        break;
         ...
         * Storyline suspend_data format: encoded string with slide states
         * Also uses DS.setVariable for slide tracking
         */
        _markStorylineSlides(suspendData) {
            const result = { data: suspendData, found: 0, marked: 0, lastSlideId: null };

            // Try to parse Storyline suspend_data
            // Format varies but often contains slide visited states
            if (suspendData) {
                try {
                    // Storyline often uses a delimited format or base64
                    // Common patterns: "slideId:state,slideId:state" or JSON
                    if (suspendData.includes('{')) {
                        // JSON format
                        const parsed = JSON.parse(suspendData);
                        if (parsed.slides || parsed.visited || parsed.v) {
                            const slides = parsed.slides || parsed.visited || parsed.v;
                            if (Array.isArray(slides)) {
                                result.found = slides.length;
                                // Mark all as viewed (typically 1 = viewed)
                    ...
                    } else if (suspendData.includes(',') || suspendData.includes('|')) {
                        // Delimited format
                        const delimiter = suspendData.includes('|') ? '|' : ',';
                        const parts = suspendData.split(delimiter);
                        result.found = parts.length;

                        // Mark each part as viewed (set to 1 or 'c' for complete)
                        const marked = parts.map((part, idx) => {
                            result.marked++;
                            result.lastSlideId = idx + 1;
                            // If part has : separator, it's key:value
                            if (part.includes(':')) {
                        ...
                        result.data = marked.join(delimiter);
                    }
                } catch (e) {
            ...
            // Also check DS object for slide count
            if (window.DS && window.DS.VO) {
                try {
                    let slideCount = 0;
                    for (const [id, obj] of Object.entries(window.DS.VO)) {
         ...
         * Rise uses JSON suspend_data with block completion states
         */
        _markRiseBlocks(suspendData) {
            const result = { data: suspendData, found: 0, marked: 0, lastSlideId: null };
            ...
            if (suspendData) {
                try {
                    ...
                    Logger.debug('Could not parse Rise suspend_data', e);
                }
            }
        ...
        async forceFullCompletion(options = {}) {
            ...
            Logger.info('Starting full course completion', options);
            ...
            try {
                ...
                // Step 3: Set final completion status
                result.completion = await this.forceCompletion({
                    apiIndex, status, score,
                    terminate: false,
    ...
    const StorylineExtractor = {
        ...
        extractFromObject(obj, source, items, depth = 0) {
            ...
            if (typeof obj === 'object' && !Array.isArray(obj)) {
                // Check for sequence/matching question types first
                const interactionType = obj.interactionType || obj.type || obj.questionType;

                // Handle sequence questions
                if (interactionType === 'sequencing' || interactionType === 'sequence' || obj.correctOrder) {
                    this.extractSequenceQuestion(obj, source, items);
                    return;
                }

                // Handle matching questions
                if (interactionType === 'matching' || (obj.source && obj.target) || (obj.statements && obj.choices)) {
                    this.extractMatchingQuestion(obj, source, items);
                    return;
                }
                ...
                if (typeof text === 'string' && text.length > 10 && !Utils.isCodeLike(text)) {
                    ...
                    let questionType = QUESTION_TYPE.MULTIPLE_CHOICE;
                    ...
                    // Text with question indicators is a QUESTION
                        ...
                        questionType = this.detectQuestionType(text, obj);
                    }
                    ...
                    if (itemType === ITEM_TYPE.QUESTION) {
        ...
        /**
         * Extract sequence question - items that need to be ordered
         */
        extractSequenceQuestion(obj, source, items) {
            const questionText = obj.questionText || obj.question || obj.caption || obj.text || 'Arrange in correct order';
            ...
            items.push({
                ...
                source: `Storyline:${source}`,
                confidence: CONFIDENCE.HIGH,
                correctOrder: obj.correctOrder || obj.correctResponsesPattern || null
            });

            // Extract sequence items (choices that can be reordered)
            const seqItems = obj.choices || obj.items || obj.options || [];
            seqItems.forEach((item, index) => {
                const itemText = item.text || item.caption || item.lmstext || item;
                if (typeof itemText === 'string' && itemText.length > 0) {
                    items.push({
                        type: ITEM_TYPE.SEQUENCE,
                        text: itemText.trim(),
                        id: item.id || `seq_${index}`,
                        displayPosition: index,
                        correctPosition: this.findCorrectPosition(item.id || index, obj.correctOrder),
                        source: `Storyline:${source}`,
                        confidence: CONFIDENCE.HIGH
                    });
                }
        ...
        /**
         * Find correct position of an item in the sequence
         */
        findCorrectPosition(itemId, correctOrder) {
            if (!correctOrder) return null;

            // correctOrder can be string like "3[,]1[,]4[,]2" or array
            if (typeof correctOrder === 'string') {
                const orderArray = correctOrder.split('[,]');
                const pos = orderArray.indexOf(String(itemId));
                return pos >= 0 ? pos : null;
            }

            if (Array.isArray(correctOrder)) {
                const pos = correctOrder.indexOf(itemId);
                return pos >= 0 ? pos : null;
            }
        ...
        /**
         * Extract matching question - pairs to be matched
         */
        extractMatchingQuestion(obj, source, items) {
            const questionText = obj.questionText || obj.question || obj.caption || obj.text || 'Match the items';
            ...
            // Extract sources (statements/prompts)
            const sources = obj.source || obj.statements || obj.prompts || [];
            sources.forEach((item, index) => {
                const itemText = item.text || item.caption || item.lmstext || item;
                if (typeof itemText === 'string' && itemText.length > 0) {
                    items.push({
                        type: ITEM_TYPE.MATCH_SOURCE,
                        text: itemText.trim(),
                        matchId: item.id || `src_${index}`,
                        source: `Storyline:${source}`,
                        confidence: CONFIDENCE.HIGH
                    });
                }
            ...
            // Extract targets (choices/responses)
            const targets = obj.target || obj.choices || obj.responses || [];
            targets.forEach((item, index) => {
                const itemText = item.text || item.caption || item.lmstext || item;
                if (typeof itemText === 'string' && itemText.length > 0) {
                    items.push({
                        type: ITEM_TYPE.MATCH_TARGET,
                        text: itemText.trim(),
                        matchId: item.id || `tgt_${index}`,
                        correctMatch: item.correctMatch || item.matchTo || null,
                        source: `Storyline:${source}`,
                        confidence: CONFIDENCE.HIGH
                    });
                }
            ...
            // Handle correctResponsesPattern for matching (format: "src1[.]tgt1[,]src2[.]tgt2")
            if (obj.correctResponsesPattern && typeof obj.correctResponsesPattern === 'string') {
                const pairs = obj.correctResponsesPattern.split('[,]');
                pairs.forEach(pair => {
                    const [srcId, tgtId] = pair.split('[.]');
                    // Find and update the target with its correct match
                    const target = items.find(i =>
                        i.type === ITEM_TYPE.MATCH_TARGET &&
                        (i.matchId === tgtId || i.text === tgtId)
                    );
                    if (target) {
                        target.correctMatch = srcId;
                    }
                });
            }
        },
        ...
        /**
         * Detect question type from text and object properties
         */
        detectQuestionType(text, obj) {
            const lower = text.toLowerCase();

            if (lower.includes('order') || lower.includes('sequence') || lower.includes('arrange')) {
                return QUESTION_TYPE.SEQUENCING;
            }
            if (lower.includes('match') || lower.includes('pair')) {
                return QUESTION_TYPE.MATCHING;
            }
            if (lower.includes('drag') || lower.includes('drop')) {
                return QUESTION_TYPE.DRAG_DROP;
            }
            if (lower.includes('true') && lower.includes('false')) {
                return QUESTION_TYPE.TRUE_FALSE;
            }
            if (lower.includes('fill in') || lower.includes('blank')) {
                return QUESTION_TYPE.FILL_IN;
            }
            if (lower.includes('select all') || lower.includes('multiple')) {
                return QUESTION_TYPE.MULTIPLE_RESPONSE;
            }

            // Check object properties
            if (obj.interactionType) {
                const typeMap = {
                    'choice': QUESTION_TYPE.MULTIPLE_CHOICE,
                    'sequencing': QUESTION_TYPE.SEQUENCING,
                    'matching': QUESTION_TYPE.MATCHING,
                    'fill-in': QUESTION_TYPE.FILL_IN,
                    'true-false': QUESTION_TYPE.TRUE_FALSE,
                    'numeric': QUESTION_TYPE.NUMERIC,
                    'likert': QUESTION_TYPE.LIKERT,
                    'long-fill-in': QUESTION_TYPE.ESSAY
                };
                return typeMap[obj.interactionType] || QUESTION_TYPE.MULTIPLE_CHOICE;
            }
    ...
    const Scanner = {
        async run() {
            ...
            try {
                ...
                this.reportProgress(1, 6, 'Discovering LMS APIs...');
                SCORMAPI.discover();

                // Step 2: Detect authoring tool
                this.reportProgress(2, 6, 'Detecting authoring tool...');
                const detectedTool = ExtractorRegistry.detectTool();
                StateManager.set('tool', detectedTool);
                Logger.info(`Detected authoring tool: ${detectedTool}`);

                // Step 3: Run tool-specific extraction
                this.reportProgress(3, 6, `Extracting from ${detectedTool}...`);
                ...
                // Step 4: Extract from Storyline accessibility DOM (if applicable)
                this.reportProgress(4, 6, 'Scanning accessibility DOM...');
                const storylineDOMItems = StorylineDOMExtractor.extract();

                // Step 5: Scan DOM for generic quiz forms
                this.reportProgress(5, 6, 'Scanning DOM for quiz forms...');
                const domQuizzes = DOMQuizExtractor.extract();
                const domItems = DOMQuizExtractor.toQAItems(domQuizzes);

                // Step 6: Analyze embedded resources
                this.reportProgress(6, 6, 'Analyzing resources...');
                ResourceDiscovery.discover();
