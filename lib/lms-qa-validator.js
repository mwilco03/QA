...
(function() {
    ...
    const MSG = Object.freeze({
        ...
        CMD_ADVANCE_MEDIA: 'LMS_QA_CMD_ADVANCE_MEDIA',
        ADVANCE_MEDIA_RESULT: 'ADVANCE_MEDIA_RESULT',
        CMD_CLICK_ADVANCE: 'LMS_QA_CMD_CLICK_ADVANCE',
    ...
    const Messenger = {
        ...
        handleCommand(type, payload) {
            ...
            switch (type) {
                ...
                case MSG.CMD_ADVANCE_MEDIA:
                    const advResult = MediaController.advanceMedia(payload || {});
                    Messenger.send(MSG.ADVANCE_MEDIA_RESULT, advResult);
                    break;

                case MSG.CMD_CLICK_ADVANCE:
                    const clickResult = MediaController.clickAdvance(payload || {});
                    Messenger.send(MSG.CLICK_ADVANCE_RESULT, clickResult);
                    break;
            }
        },
    ...
    const SCORMAPI = {
        ...
        discover() {
            ...
            // ADL find-API algorithm: walk parent chain to top, then walk opener
            // chain. Workday hosts the SCO inside a popup window whose top frame
            // owns the API object — searching parent + opener.top covers it.
            const seenWindows = new WeakSet();

            const collectChain = (start, label) => {
                const chain = [];
                let cur = start;
                let depth = 0;
                while (cur && depth <= CONFIG.MAX_API_SEARCH_DEPTH) {
                    try {
                        if (seenWindows.has(cur)) break;
                        seenWindows.add(cur);
                        chain.push({ name: `${label}${depth ? '.parent'.repeat(depth) : ''}`, obj: cur });
                        if (cur.parent === cur) break;
                        cur = cur.parent;
                    } catch (e) { break; /* Cross-origin */ }
                    depth++;
                }
                return chain;
            };

            const searchLocations = collectChain(window, 'window');

            try {
                if (window.opener && window.opener !== window) {
                    searchLocations.push(...collectChain(window.opener, 'opener'));
                    try {
                        if (window.opener.top && window.opener.top !== window.opener) {
                            searchLocations.push({ name: 'opener.top', obj: window.opener.top });
                        }
            ...
            // Recurse into same-origin child iframes. Workday's SCO is typically
            // 2-3 frames down from the launched popup's top window, so the API
            // adapter on `window` of the launched top is only visible if we
            // either start from inside the SCO frame (handled by parent walk
            // above when content script runs in the SCO frame) OR walk down
            // from the top when running there.
            for (const { name, obj } of searchLocations) {
                try {
                    const childApis = this.searchChildFrames(obj, name, 0);
                    found.push(...childApis);
                } catch (e) { /* Cross-origin */ }
            }
        ...
        searchChildFrames(win, path, depth) {
            if (!win || depth > CONFIG.MAX_API_SEARCH_DEPTH) return [];
            const found = [];
            ...
            iframes.forEach((iframe, i) => {
                let childWin;
                try { childWin = iframe.contentWindow; } catch (e) { return; }
                if (!childWin) return;

                const label = `${path}.frames[${iframe.id || iframe.name || i}]`;
                try {
                    found.push(...this.searchObject(childWin, label, 0));
                    found.push(...this.searchStandaloneFunctions(childWin, label));
                } catch (e) { /* Cross-origin */ }

                // Recurse one more level
                try {
                    found.push(...this.searchChildFrames(childWin, label, depth + 1));
                } catch (e) { /* Cross-origin */ }
            });
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // SECTION 6.5: MEDIA & NAVIGATION CONTROLS
    // Walk same-origin frame tree, locate <video>/<audio> elements and
    // advance/forward buttons. Used to clear video-gated module steps in
    // LMS players (e.g. Workday Learning lessons).
    // ═══════════════════════════════════════════════════════════════════════════

    const MediaController = {
        ADVANCE_TEXT_PATTERNS: [
            /^next$/i, /^continue$/i, /^proceed$/i, /^skip$/i,
            /^next\s*(slide|module|lesson|page|section|>>?|→)?$/i,
            /^continue\s*(>>?|→)?$/i,
            /^mark\s+(as\s+)?complete$/i,
            /^complete$/i, /^finish$/i, /^submit$/i, /^done$/i,
            /^i\s+agree$/i, /^acknowledge$/i, /^got\s+it$/i,
            /^play$/i, /^start$/i
        ],
        ...
        collectMediaElements() {
            ...
            const walk = (doc, path) => {
                ...
                doc.querySelectorAll('video, audio').forEach((el, i) => {
                    items.push({ el, path: `${path}.${el.tagName.toLowerCase()}[${i}]` });
                });
                doc.querySelectorAll('iframe, frame').forEach((iframe, i) => {
                    let childDoc;
                    try { childDoc = iframe.contentDocument; } catch (e) { return; }
                    if (childDoc) walk(childDoc, `${path}.frames[${iframe.id || iframe.name || i}]`);
                });
            };
        ...
        collectButtons() {
            const items = [];
            const selector = 'button, [role="button"], a[href], input[type="button"], input[type="submit"]';
            const walk = (doc, path) => {
                if (!doc || !doc.querySelectorAll) return;
                doc.querySelectorAll(selector).forEach((el, i) => {
                    items.push({ el, path: `${path}[${i}]` });
                });
                doc.querySelectorAll('iframe, frame').forEach((iframe, i) => {
                    let childDoc;
                    try { childDoc = iframe.contentDocument; } catch (e) { return; }
        ...
        advanceMedia(options = {}) {
            const { mode = 'end' } = options;
            const items = this.collectMediaElements();
            const results = [];

            items.forEach(({ el, path }) => {
                try {
                    if (el.muted === false) el.muted = true; // most autoplay policies require this
                    if (mode === 'fast') {
                        const target = Math.min(16, Math.max(el.playbackRate || 1, 8));
                        el.playbackRate = target;
                        const playPromise = el.play?.();
                        if (playPromise?.catch) playPromise.catch(() => {});
                        results.push({ path, action: `playbackRate=${target}`, ok: true });
                    } else {
                        // 'end' mode: seek to end and fire ended
                        if (isFinite(el.duration) && el.duration > 0) {
                            el.currentTime = Math.max(0, el.duration - 0.1);
                        }
                        try { el.pause?.(); } catch (e) {}
                        // Fire timeupdate + ended so listeners on the player update
                        el.dispatchEvent(new Event('timeupdate', { bubbles: true }));
                        el.dispatchEvent(new Event('ended', { bubbles: true }));
                        results.push({
                            path,
                            action: 'seek-to-end+ended',
                            duration: el.duration,
                            ok: true
                        });
                    }
                } catch (e) {
                    results.push({ path, action: 'error', error: e.message, ok: false });
                }
            });
        ...
        clickAdvance(options = {}) {
            const { allMatching = false } = options;
            ...
            buttons.forEach(({ el, path }) => {
                ...
                const label = this.getButtonLabel(el);
                if (!label) return;
                if (this.ADVANCE_TEXT_PATTERNS.some(p => p.test(label))) {
                    matches.push({ el, label, path });
                }
            });

            if (matches.length === 0) {
                return { clicked: 0, candidates: 0 };
            }

            const toClick = allMatching ? matches : [matches[0]];
            toClick.forEach(m => {
                try { m.el.click(); } catch (e) { /* ignore */ }
            });

            return {
                clicked: toClick.length,
                candidates: matches.length,
                labels: toClick.map(m => m.label)
            };
        }
    };
    ...
    window.LMS_QA = {
        ...
        advanceMedia: (opts) => MediaController.advanceMedia(opts),
        clickAdvance: (opts) => MediaController.clickAdvance(opts)
    };
