...
(function() {
    ...
    const CONTENT_PATTERNS = {
        questions: [
            /(?:question|prompt|ask|query)[\s:]*["']?([^"'\n]{10,200})/gi,
            /(?:^|\n)\s*(?:\d+[\.\)]\s*)?([^?\n]{15,150}\?)/gm,
            /what\s+(?:is|are|was|were|do|does|did|would|could|should)\s+[^?\n]{10,100}\?/gi
        ],
        answers: [
            /(?:answer|correct|solution|response)[\s:]*["']?([^"'\n]{5,200})/gi,
            /(?:^|\n)\s*[a-d][\.\)]\s*([^\n]{5,150})/gmi
        ],
        correct: [
            /correct[\s_-]*(?:answer|response|option)?[\s:]*["']?([^"'\n]{5,200})/gi,
            /is[\s_-]*correct["']?\s*:\s*true/gi,
            /["']?correct["']?\s*:\s*true/gi
        ]
    };
    ...
    const SCORMAPI = {
        signatures: {
            ...
            [LMS_STANDARD.XAPI]: {
                ...
                optional: ['getStatement', 'getStatements']
            },
            [LMS_STANDARD.AICC]: {
        ...
        searchObject(obj, path, depth) {
            ...
            const found = [];
            const apiNames = ['API', 'API_1484_11', 'SCORM_API', 'pipwerks'];
    ...
    const ResourceDiscovery = {
        ...
        addResource(map, url, source) {
            ...
            if (/jquery|angular|react|vue|lodash|backbone|moment/i.test(url)) {
                return;
            }
        ...
        extractFromJson(obj, source, items, depth = 0) {
            ...
            if (typeof obj === 'object' && !Array.isArray(obj)) {
                const questionKeys = ['question', 'prompt', 'text', 'stem', 'query'];
                const answerKeys = ['answer', 'response', 'correct', 'options', 'choices'];
                ...
                for (const key of answerKeys) {
                    ...
                    if (typeof value === 'string' && value.length > 2) {
                        items.push({
                            ...
                            text: value.trim(),
                            source: `JSON:${source}`,
                            correct: key === 'correct' || obj.isCorrect === true,
                            confidence: CONFIDENCE.HIGH
                        });
