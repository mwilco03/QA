 ...
 * Built: 2026-01-13T17:04:22.585Z
 *
...
(function () {
    ...
    const ITEM_TYPE$1 = Object.freeze({
        QUESTION: 'question',
        ANSWER: 'answer',
    ...
    function log(level, levelName, message, data) {
        ...
        StateManager$1.append('logs', entry);

        const logs = StateManager$1.get('logs');
        if (logs.length > CONFIG$1.MAX_LOGS) {
            StateManager$1.set('logs', logs.slice(-CONFIG$1.MAX_LOGS));
        }
    ...
    const Logger = {
        ...
        getLogs() {
            return StateManager$1.get('logs');
        },
    ...
    const Utils = {
        ...
        async fetchWithTimeout(url, timeout = CONFIG$1.MAX_FETCH_TIMEOUT) {
            ...
            textMatches.forEach(m => {
                ...
                if (captionMatch) {
                    ...
                    if (txt.length > 20 && Utils.isNaturalLanguage(txt) && !Utils.isCodeLike(txt)) {
                        ...
                        items.push({
                            type: isQuestion ? ITEM_TYPE$1.QUESTION : ITEM_TYPE$1.ANSWER,
                            text: txt,
                ...
                timeline.cues.forEach(cue => {
                    if (cue.text && typeof cue.text === 'string') {
                        ...
                        if (text.length > 15 && Utils.isNaturalLanguage(text)) {
                            items.push({
                                type: this.isQuestionText(text) ? ITEM_TYPE$1.QUESTION : ITEM_TYPE$1.ANSWER,
                                text: text,
        ...
        extractFromQuestionObject(q, source, items) {
            ...
            if (q.correctAnswer || q.correctResponse) {
                ...
                if (typeof correct === 'string') {
                    ...
                    items.forEach(item => {
                        if (item.type === ITEM_TYPE$1.ANSWER && item.text === correct) {
                            item.correct = true;
                        }
                    });
                }
    ...
    const SCORMAPI$1 = {
        ...
        discover() {
            ...
            StateManager$1.set('apis', unique);
        ...
        searchObject(obj, path, depth) {
            if (!obj || depth > CONFIG$1.MAX_API_SEARCH_DEPTH) return [];
        ...
        test(apiIndex = 0) {
            const apis = StateManager$1.get('apis');
            if (apiIndex >= apis.length) {
                Messenger$1.send(MSG.TEST_RESULT, { success: false, error: 'No API at index' });
                return;
            }
            ...
            Messenger$1.send(MSG.TEST_RESULT, result);
            return result;
        },
        ...
        setCompletion(options = {}) {
            ...
            if (apiIndex >= apis.length) {
                Messenger$1.send(MSG.SET_COMPLETION_RESULT, { success: false, error: 'No API available' });
                return;
            }
            ...
            Messenger$1.send(MSG.SET_COMPLETION_RESULT, result);
            return result;
        },
        ...
        async forceCompletion(options = {}) {
            ...
            // ─────────────────────────────────────────────────────────
            let apis = StateManager$1.get('apis');
            if (!apis || apis.length === 0) {
                this.discover();
                apis = StateManager$1.get('apis');
            }
            ...
            Messenger$1.send(MSG.FORCE_COMPLETION_RESULT, result);
            return result;
        },
        ...
        async completeAllObjectives(options = {}) {
            ...
            Messenger$1.send(MSG.OBJECTIVES_COMPLETE, result);
            return result;
        },
        ...
        async markAllSlidesViewed(options = {}) {
            ...
            Messenger$1.send(MSG.SLIDES_MARKED, result);
            return result;
        },
        ...
        async forceFullCompletion(options = {}) {
            ...
            Messenger$1.send(MSG.FULL_COMPLETION_RESULT, result);
            return result;
        },
        ...
        async tryAllCompletionMethods(options = {}) {
            ...
            Messenger$1.send(MSG.FORCE_COMPLETION_RESULT, results);
    ...
    const Scanner = {
        async run() {
            if (StateManager$1.get('scanning')) {
                Logger.warn('Scan already in progress');
                return;
            }

            const endTimer = Logger.time('Full scan');
            StateManager$1.reset();
            StateManager$1.set('scanning', true);

            Messenger$1.send(MSG.SCAN_STARTED);
            ...
            try {
                ...
                StateManager$1.set('tool', detectedTool);
                StateManager$1.set('frameworkEvidence', detection.evidence);
                ...
                // Store results
                StateManager$1.set('qa', allItems);
                StateManager$1.set('groupedQuestions', groupedQuestions);
                StateManager$1.set('scanning', false);
                StateManager$1.set('lastScan', Date.now());
                ...
                Messenger$1.send(MSG.SCAN_COMPLETE, report);

            } catch (error) {
                StateManager$1.set('scanning', false);
                Logger.error('Scan failed', { error: error.message });
                Messenger$1.send(MSG.SCAN_ERROR, { error: error.message });
            }
        },

        reportProgress(step, total, message) {
            Messenger$1.send(MSG.PROGRESS, { step, total, message });
        }
    };
    ...
    const Reporter$1 = {
        generate() {
            const qa = StateManager$1.get('qa');
            const apis = StateManager$1.get('apis');
            const resources = StateManager$1.get('resources');
            const logs = StateManager$1.get('logs');
            const warnings = StateManager$1.get('warnings');
            const tool = StateManager$1.get('tool');
            const frameworkEvidence = StateManager$1.get('frameworkEvidence') || [];
            const groupedQuestions = StateManager$1.get('groupedQuestions') || [];
    ...
    if (window.__LMS_QA_INJECTED__) {
        ...
        Logger.info(`LMS QA Validator v${VERSION$1} initializing`);

        // ═══════════════════════════════════════════════════════════════════════════
        // COMMAND HANDLER
        // ═══════════════════════════════════════════════════════════════════════════

        Messenger$1.listen(async (type, payload) => {
            switch (type) {
                case MSG.CMD_SCAN:
                    StateManager$1.set('scanning', true);
                    Messenger$1.send(MSG.SCAN_STARTED, {});
                    try {
                        ...
                        StateManager$1.set('qa', result.items);
                        Messenger$1.send(MSG.SCAN_COMPLETE, {
                            items: result.items,
                        ...
                        Messenger$1.send(MSG.SCAN_ERROR, { error: e.message });
                    } finally {
                        StateManager$1.set('scanning', false);
                    }
                    break;
                ...
                case MSG.CMD_GET_STATE:
                    Messenger$1.send(MSG.STATE, StateManager$1.get());
                    break;

                case MSG.CMD_GET_CMI_DATA:
                    const cmiData = SCORMAPI$1.getCmiData();
                    Messenger$1.send(MSG.CMI_DATA, cmiData);
                    break;

                case MSG.CMD_DETECT_APIS:
                    const apis = SCORMAPI$1.discover();
                    Messenger$1.send(MSG.APIS_DETECTED, { apis });
                    break;

                case MSG.CMD_TEST_API:
                    SCORMAPI$1.test(payload?.apiIndex || 0);
                    break;

                case MSG.CMD_SET_COMPLETION:
                    SCORMAPI$1.setCompletion(payload || {});
                    break;

                case MSG.CMD_FORCE_COMPLETION:
                    await SCORMAPI$1.forceCompletion(payload || {});
                    break;

                case MSG.CMD_AUTO_SELECT:
                    const count = DOMQuizExtractor.autoSelect();
                    Messenger$1.send(MSG.AUTO_SELECT_RESULT, { count });
                    break;

                case MSG.CMD_EXPORT:
                    const exportData = Exporter.export(payload?.format || 'json');
                    Messenger$1.send('EXPORT_RESULT', exportData);
                    break;

                case MSG.CMD_SEED_EXTRACT:
                    const seedResult = await SeedExtractor.extractFromSeed(payload?.seedText);
                    Messenger$1.send(MSG.SEED_EXTRACT_RESULT, seedResult);
                    break;
        ...
        window.LMS_QA = {
            ...
            // State
            getState: () => StateManager$1.get(),
            getQA: () => StateManager$1.get('qa'),
            getAPIs: () => StateManager$1.get('apis'),
            ...
            // API operations
            testAPI: (index) => SCORMAPI$1.test(index),
            setCompletion: (opts) => SCORMAPI$1.setCompletion(opts),
            forceCompletion: (opts) => SCORMAPI$1.forceCompletion(opts),
        ...
        Messenger$1.send(MSG.READY, { version: VERSION$1, url: window.location.href });
