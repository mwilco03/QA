    ...
    const StorylineDOMExtractor = {
        ...
        extractFromAccText(doc, processed) {
            ...
            elements.forEach(el => {
                ...
                // Skip if looks like code or navigation
                if (Utils.isCodeLike(text)) return;
                if (/^(next|prev|back|submit|continue|menu)$/i.test(text)) return;
        ...
        extractFromAccShadow(doc, processed) {
            ...
            radioGroups.forEach((radios, groupName) => {
                radios.forEach(radio => {
                    ...
                    if (!labelText || labelText.length < 3 || processed.has(labelText)) return;
