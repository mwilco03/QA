...
(function() {
    ...
    const LMSExtractor = {
        ...
        extractFromDS(ds, questions) {
            try {
                // Storyline DS uses GetVar/SetVar methods
                if (typeof ds.GetVar !== 'function') {
                    console.log('[Extractor] DS has no GetVar method, checking properties');
                    // Fallback: enumerate object properties
                    for (const [key, value] of Object.entries(ds)) {
                        if (typeof value !== 'function') {
                            questions.push({ source: 'DS_Property', key, value });
                        }
                    }
                    return;
                }

                // Known Storyline quiz variable patterns
                const quizVarPatterns = [
                    'Results.ScorePoints', 'Results.ScorePercent', 'Results.PassPercent',
                    'Results.PassPoints', 'Results.QuizPointsAvailable', 'Results.QuizPointsScored',
                    'Score', 'ScorePercent', 'ScorePoints', 'PassFail', 'PassPercent',
                    'QuizResult', 'Passed', 'Failed', 'Complete'
                ];

                // Try standard quiz variables
                quizVarPatterns.forEach(varName => {
                    try {
                ...
                // Try numbered question variables (Q1_Answer, Q2_Correct, etc.)
                for (let i = 1; i <= 50; i++) {
                    ['Answer', 'Correct', 'Response', 'Result', 'Points', 'Selected'].forEach(suffix => {
                        try {
                            const varName = `Q${i}_${suffix}`;
                            const val = ds.GetVar(varName);
                            if (val !== undefined && val !== null && val !== '') {
                                questions.push({ source: 'DS', key: varName, value: val });
                            }
                ...
                console.log(`[Extractor] Extracted ${questions.length} DS variables`);
            } catch (e) {
        ...
        extractFromPlayer(player, questions) {
            try {
                if (typeof player.GetVar !== 'function') {
                    console.log('[Extractor] Player has no GetVar method');
                    return;
                }

                // Standard Storyline player variables
                const playerVars = [
                    // Quiz results
                    'Results.ScorePoints', 'Results.ScorePercent', 'Results.PassPercent',
                    'Results.PassPoints', 'Results.QuizPointsAvailable', 'Results.QuizPointsScored',
                ...
                // Try to access internal variable store if available
                if (player._variables || player.variables) {
                    const vars = player._variables || player.variables;
                    console.log(`[Extractor] Found player variables store with ${Object.keys(vars).length} entries`);
                ...
                // Try numbered question variables
                for (let i = 1; i <= 50; i++) {
                    ['Answer', 'Correct', 'Response', 'Result', 'Points', 'Selected', 'Choice'].forEach(suffix => {
                        try {
                            const varName = `Question${i}_${suffix}`;
                            const val = player.GetVar(varName);
                            if (val !== undefined && val !== null && val !== '') {
                                questions.push({ source: 'Player', key: varName, value: val });
                            }
                ...
                console.log(`[Extractor] Extracted ${questions.length} Player variables`);
            } catch (e) {
        ...
        parseStorylineData(data, questions) {
            // Parse Storyline slide data from globalProvideData format
            // This matches the structure in storyline-console-extractor.js
            try {
                const correctStateIndicators = [
                    '_Review', '_Selected_Review', 'Correct', 'Right', 'True',
                    'Selected_Correct', 'Drop_Correct', 'Drag_Correct'
                ];
                const answerAccTypes = [
                ...
                const extractText = (obj) => {
                    ...
                    if (obj?.rawText) return obj.rawText.trim();
                    if (typeof obj?.text === 'string') return obj.text.trim();
                    if (typeof obj?.title === 'string') return obj.title.trim();
                    if (typeof obj?.accText === 'string') return obj.accText.trim();
                    return '';
                };
                ...
                const isCorrect = (obj) => {
                    ...
                    return obj.states.some(s =>
                        correctStateIndicators.some(ind =>
                ...
                const searchObject = (obj, slideId = 'unknown') => {
                    ...
                    if (accType && answerAccTypes.includes(accType) && text) {
                        // Skip navigation buttons
                        const lowerText = text.toLowerCase();
                        if (accType === 'button' && excludeButtons.some(exc => lowerText.includes(exc))) {
                            return;
                        }

                        // Skip duplicates
                        if (questions.some(q => q.text === text && q.slideId === slideId)) {
                            return;
                        }
                    ...
                    for (const key in obj) {
                        if (Array.isArray(obj[key])) {
                            obj[key].forEach(item => searchObject(item, slideId));
                        } else if (typeof obj[key] === 'object') {
                            searchObject(obj[key], slideId);
                        }
                    }
                };

                // Handle both single slide data and full course data
                if (data.slideLayers || data.objects || data.timeline) {
                    // Single slide format
                    ...
                    for (const scene of data.scenes) {
                        for (const slide of scene.slides || []) {
                            searchObject(slide, slide.id);
                        }
                    }
                } else {
                    // Unknown format - try recursive search
                    searchObject(data);
                }

                console.log(`[Extractor] Parsed ${questions.length} items from Storyline data`);
            } catch (e) {
        ...
        async extractTLA() {
            ...
            // Method 2: Extract from xAPI wrapper if available
            const tlaGlobals = WindowManager.findTLAGlobal();
            if (tlaGlobals.ADL) {
                console.log('[Extractor] Extracting from ADL xAPI Wrapper');
        ...
        async extractFromXAPI(ADL, questions) {
            try {
                ...
                if (!wrapper) {
                    console.log('[Extractor] No XAPIWrapper found in ADL');
                    return;
                }

                // Try to get statements from the LRS
                // Get configuration to find the endpoint
                const config = wrapper.lrs || wrapper.Config || {};
                console.log('[Extractor] xAPI endpoint:', config.endpoint);

                // Method 1: Try to get statements directly if getStatements is available
                if (typeof wrapper.getStatements === 'function') {
                    try {
                        const result = wrapper.getStatements({
                            verb: 'http://adlnet.gov/expapi/verbs/answered',
                            limit: 100
                        });
                        if (result?.statements) {
                            console.log(`[Extractor] Found ${result.statements.length} xAPI answered statements`);
                            result.statements.forEach(stmt => {
                                questions.push({
                                    source: 'xAPI',
                                    id: stmt.object?.id,
                                    verb: stmt.verb?.display?.['en-US'] || stmt.verb?.id,
                                    result: stmt.result?.response,
                        ...
                        console.warn('[Extractor] getStatements error:', e.message);
                    }
                }

                // Method 2: Check for cached/queued statements
                if (wrapper.statements || wrapper._statements) {
                    const statements = wrapper.statements || wrapper._statements;
                    if (Array.isArray(statements) && statements.length > 0) {
                        console.log(`[Extractor] Found ${statements.length} cached xAPI statements`);
                ...
                // Method 3: Try to get state data
                if (typeof wrapper.getState === 'function') {
                    try {
                        ...
                        if (state) {
                            console.log('[Extractor] Found xAPI state data');
                            questions.push({
                                source: 'xAPI_State',
                                type: 'progress',
                                data: state
                ...
                // Method 4: Check for activity profile
                if (typeof wrapper.getActivityProfile === 'function') {
                    try {
                        const activityId = config.activityId || window.location.href.split('?')[0];
                        const profile = wrapper.getActivityProfile(activityId, 'quiz-results');
                        if (profile) {
                            console.log('[Extractor] Found xAPI activity profile');
                            questions.push({
                                source: 'xAPI_Profile',
                                type: 'quiz-results',
                                data: profile
                ...
                console.warn('[Extractor] xAPI extraction error:', e);
            }
        },
