/**
 * LMS Course Extractor & Completion Script
 *
...
(function() {
    ...
    const LMSExtractor = {
        ...
        /**
         * Parse Storyline data structure for quiz content
         * Handles data from globalProvideData or window._storylineData
         */
        parseStorylineData(data, questions) {
            ...
            const search = (obj, path = '') => {
                ...
                // Look for quiz/question indicators
                const accType = obj.accType;
                const hasChoices = obj.choices && Array.isArray(obj.choices);
                const hasInteraction = obj.type && (obj.type.includes('choice') || obj.type.includes('quiz'));

                // Extract text content
                const getText = (o) => {
                    if (o.textLib?.[0]?.vartext?.blocks) {
                        return o.textLib[0].vartext.blocks
                            .flatMap(b => b.spans?.map(s => s.text) || [])
                            .join('');
                    }
                    return o.rawText || o.text || o.title || o.label || o.accText || '';
                };

                // Check for answer elements (radiobutton, checkbox, etc.)
                if (accType && ['radiobutton', 'checkbox', 'button', 'hotspot'].includes(accType)) {
                    const text = getText(obj).trim();
                    if (text && text.length > 0) {
                        // Check for correct state indicators
                        const hasCorrectState = obj.states?.some(s =>
                            s.name && (s.name.includes('_Review') || s.name.includes('Correct'))
                        );
                        questions.push({
                            ...
                            text,
                            accType,
                            correct: hasCorrectState || false,
                            states: obj.states?.map(s => s.name) || []
                        });
                    }
                }

                // Check for interaction objects with choices
                if (hasChoices) {
                    obj.choices.forEach((choice, i) => {
                        questions.push({
                            source: 'StorylineData:interaction',
                            path: `${path}.choices[${i}]`,
