...
(function() {
    ...
    // Safe cross-origin check for top frame detection
    let isTopFrame = true;
    try {
    ...
    if (!isTopFrame && !isForceInjected) {
        ...
        const iframeMessageHandler = (e) => {
            ...
            if (e.data?.type === 'LMS_SELECTOR_CLEANUP') {
                // Allow cleanup from top frame
                window.removeEventListener('message', iframeMessageHandler);
                delete window.__LMS_SELECTOR_IFRAME_HANDLER__;
            }
        };

        // Store reference for external cleanup if needed
        window.__LMS_SELECTOR_IFRAME_HANDLER__ = iframeMessageHandler;

        // Only add listener if not already added
        if (!window.__LMS_SELECTOR_LISTENER_ADDED__) {
            window.addEventListener('message', iframeMessageHandler);
            window.__LMS_SELECTOR_LISTENER_ADDED__ = true;
        }
    ...
    const Selector = {
        ...
        deactivate() {
            ...
            // Send cleanup message to iframes to remove their listeners
            document.querySelectorAll('iframe').forEach(iframe => {
                try {
                    iframe.contentWindow?.postMessage({ type: 'LMS_SELECTOR_CLEANUP' }, '*');
                } catch (e) {
    ...
    // Auto-activate on load - the script is only injected when activation is requested
    // This avoids race conditions with message passing
    setTimeout(() => {
        if (State.step === STEP.IDLE) {
            log('Auto-activating selector');
            Selector.activate();
        }
    }, 50);
