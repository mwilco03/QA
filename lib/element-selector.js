...
(function() {
    ...
    // Prevent double injection - cleanup any existing instance first
    if (window.__LMS_SELECTOR_INJECTED__) {
        // Clean up previous instance before re-injecting
        if (window.LMS_QA_SELECTOR?.deactivate) {
            window.LMS_QA_SELECTOR.deactivate();
        }
    ...
    const STYLES = `
        ...
        /* Override pointer-events on page elements during selection */
    ...
    const RuleExtractor = {
        ...
        extract(rule) {
            ...
            try {
                ...
                const questionElements = Array.from(document.querySelectorAll(rule.questionSelector));
                const answerElements = Array.from(document.querySelectorAll(rule.answerSelector));
                const correctElements = rule.correctSelector
                    ? Array.from(document.querySelectorAll(rule.correctSelector))
                    : [];
